...
exports.isAnswerCorrect = exports.areWordsRoughlyEqual = exports.formatAnswer = void 0;
var dictionary = require("dictionary-en");
const nspell_1 = __importDefault(require("nspell"));
dictionary(onLoadDictionary);
let nspell = undefined;
...
const equalOrMinorTypo = (given, answer) => {
...
const areWordsRoughlyEqual = (given, answer) => {
    const givenWords = given.split(" ");
    const answerWords = answer.split(" ");
    if (givenWords.length === answerWords.length &&
...
exports.areWordsRoughlyEqual = areWordsRoughlyEqual;
const isAnswerCorrect = (given, official) => {
    ...
    for (let candidateAnswer of officialAnswers) {
        if (exports.areWordsRoughlyEqual(given, candidateAnswer)) {
            return "correct";
        }
    }
    for (let prompt of prompts) {
        if (exports.areWordsRoughlyEqual(given, prompt)) {
            return "prompt";
        }
    }
    for (let candidateAnswer of officialAnswers) {
...
const test = (officialAnswer, givenAnswer, expected = "correct") => {
    const obj = exports.formatAnswer(officialAnswer);
    const result = exports.isAnswerCorrect(givenAnswer, obj);
    if (expected === result) {
        return true;
    }
    else {
...
function onLoadDictionary(err, dict) {
    if (err) {
        throw err;
    }
    nspell = nspell_1.default(dict);
    ...
    test("Jean Auguste Dominique <strong>Ingres</strong> &lt;Visual Arts, GY&gt;&lt;ed. AH&gt;", "Ingres");
    test("Jean Auguste Dominique <strong>Ingres</strong> &lt;Visual Arts, GY&gt;&lt;ed. AH&gt;", "jean auguste dominique ingres");
    test("Answer: capital punishment (or the death penalty, execution, or equivalents; prompt on lethal injection before mentioned)", "death penalty");
    test("Answer: capital punishment (or the death penalty, execution, or equivalents; prompt on lethal injection before mentioned)", "the death penalty");
    test("Answer: capital punishment (or the death penalty, execution, or equivalents; prompt on lethal injection before mentioned)", "execution");
    test("Answer: capital punishment (or the death penalty, execution, or equivalents; prompt on lethal injection before mentioned)", "lethal injection", "prompt");
    test("Answer: Madame Butterfly or Madama Butterfly (accept Cio-Cio San)", "madame butterfly");
    test("Answer: Madame Butterfly or Madama Butterfly (accept Cio-Cio San)", "madama butterfly");
    test('Answer: rectangles [do not accept or prompt on "square"]', "rectangle");
    test("Answer: Avogadro's number or Avogadro's constant. (prompt on 6.02 times 1023 before mentioned)", "Avogadro's constant");
    test("Answer: God [prompt on answers like divinity, the deity, or divine will] &lt;JB Philosophy&gt;", "god");
    test("Answer: God [prompt on answers like divinity, the deity, or divine will] &lt;JB Philosophy&gt;", "gd");
    test("Answer: God [prompt on answers like divinity, the deity, or divine will] &lt;JB Philosophy&gt;", "f", "no");
